...
import {uploadInviteFun, inviteDetailFun, AddInviteFun} from 'inviteApi'
...
class EditInvite extends Component {
		...
		inviteDetailFun(params).then(res => {
			if (res.result === 'success') {
				...
				for (let i in materialsData) {
					materialsData[i].key = Number(i)
				}
				res.data.attachmentList.forEach((item, index) => {
					attachmentListData.push({
				...
				this.setState({
					...
					calibrationTime: res.data.calibrationTime,
					planTime: res.data.planTime,
					materials: materialsData,
					companyImage: res.data.companyImage,
					projectImage: res.data.projectImage,
					fileList: attachmentListData,
					count: res.data.materials.length,
				});
			} else {
				console.log(res.msg)
			}
	...
	addInvite = () => {
		...
		//变更标书
		uploadInviteFun(params).then(res => {
			if (res.result === 'success') {
				Modal.confirm({
					title: '提示',
					content: '发布成功！你可在”个人中心-我的招标“中找到本标书',
					cancelButtonProps: {style: {display: 'none'}},
					okText: '我知道啦!',
					onOk: () => {
						Router.push({pathname: '/invite/mine/my-invite-detail', query: {id: this.props.router.query.id}})
					}
				});
			} else {
				console.log(res.msg)
...
//保存草稿
				...
				if (!err) {
					...
					uploadInviteFun(params).then(res => {
						if (res.result === 'success') {
							Modal.confirm({
								title: '提示',
								content: '发布成功！你可在”个人中心-我的招标-草稿“中找到本标书',
								cancelButtonProps: {style: {display: 'none'}},
								okText: '我知道啦!',
								onOk: () => {
	...
	render() {
		const {detail, materials, companyImage, projectImage, isValidate, defaultFileList} = this.state;
		...
		return (
			<Layout title='变更招标公告' menuIndex={'invite'}>
				<section className='bg-edit-invite' />
				<section className="bg-white ptb4" style={{zIndex: '9', position: 'relative', marginTop: '200px'}}>
					<div className="large text-darkgrey prl6">变更招标公告</div>
					<Form {...formItemLayout} colon={false} onSubmit={this.todoAddInvite} className="mt4">
						<Form.Item label="招标类型">
							{getFieldDecorator('invitationType', {
								initialValue: detail.invitationType,
								rules: [
									{required: isValidate, message: 'Please input your username!'},
								],
							})(
								<Radio.Group>
						...
						<Form.Item label="集团单位">
							{getFieldDecorator('conglomerateName', {
								initialValue: detail.conglomerateName,
								rules: [
									{max: 30, message: '最多30个字符!'},
								],
							})(
						...
						<Form.Item label="项目地址">
							<Province isValidate={isValidate} form={this.props.form}
							          province={'projectProvince'}
							          city={'projectCity'}
							          area={'projectArea'}
							          defaultValue={[detail.projectProvince, detail.projectCity, detail.projectArea]}
							/>
						</Form.Item>
						...
						<Row>
							<Col span={10} style={{marginLeft: '9.666666%'}}><Form.Item label="截标日期">
								{getFieldDecorator('endTime', {
									...
									rules: [{required: isValidate, message: '请输入截标日期'}],
								})(
									<DatePicker
										showTime
										showToday={false}
										...
										//defaultValue={moment(detail.endTime)}
										//locale={locale}
							...
							<Col span={11}>
								<Form.Item label="定标日期">
									{getFieldDecorator('calibrationTime', {
										//initialValue: moment(detail.calibrationTime),
										initialValue: this.state.calibrationTime ? moment(this.state.calibrationTime) : undefined,
										rules: [
										...
										<DatePicker
											...
											//defaultValue={moment(detail.calibrationTime)}
						...
						<Row>
							<Col span={10} style={{marginLeft: '9.666666%'}}><Form.Item label="保证金">
								{getFieldDecorator('margin', {
									initialValue: detail.margin?String(detail.margin):'',
									rules: [
						...
						<Form.Item label="预计进场日期">
							{getFieldDecorator('planTime', {
								initialValue: this.state.planTime ? moment(this.state.planTime) : undefined,
								rules: [
						...
						<Form.Item label="标的物类型">
							{getFieldDecorator('materialType', {
								initialValue: detail.materialType,
								rules: [
						...
						<Form.Item label="标的物明细">
							<div className="text-right">
								<Button type='primary' className="bg-primary-linear" onClick={this.addMaterial}>添加标的物</Button>
							</div>
									...
									render={(text, record, index) => (
										<Form.Item style={{marginBottom: '0'}}>
											{getFieldDecorator(`materials[${record.key}].quantity`, {
												...
												initialValue: record['quantity'] ? record['quantity'].toString() : '',
											})(
												<Input size="large" />
											)}
										</Form.Item>
						...
						<Form.Item label='报价方式'>
							{getFieldDecorator('quotingWay', {
								initialValue: detail.quotingWay,
								rules: [
						...
						<Form.Item label='支付说明'>
							{getFieldDecorator('paymentInstructions', {
								initialValue: detail.paymentInstructions,
								rules: [{required: isValidate, message: '请输入招标单位对款项支付的说明!'}, {max: 500, message: '最多500个字符'}],
							})(
							...
							<Col span={10} style={{marginLeft: '9.666666%'}}>
								<Form.Item label='需要送样'>
									{getFieldDecorator('isSpecimen', {
										initialValue: detail.isSpecimen,
									})(
										<Radio.Group>
							...
							<Col span={10} style={{marginLeft: '4.666666%'}}>
								<Form.Item label="送货上门">
									{getFieldDecorator('isDelivery', {
										initialValue: detail.isDelivery,
									})(
										<Radio.Group>
											<Radio value={1}>需要</Radio>
											<Radio value={0}>不需要</Radio>
						...
						<Form.Item label="收货地址">
							<Province form={this.props.form} province={'receiverProvince'} city={'receiverCity'} area={'receiverArea'}
							          defaultValue={[detail.receiverProvince, detail.receiverCity, detail.receiverArea]} />
						...
						<Form.Item label='详细地址'>
							{getFieldDecorator('receiverAddress', {
								initialValue: detail.receiverAddress,
								rules: [
									{required: isValidate, message: '请输入详细地址!'},
									{max: 50, message: '最多50个字符'},
									{whitespace: true, message: '请输入详细地址!'},
							...
							<Col span={10} style={{marginLeft: '9.666%'}}>
								<Form.Item label='联系人'>
									{getFieldDecorator('linkman', {
										...
										rules: [{required: isValidate, message: '请输入联系人姓名!'}, {whitespace: true, message: '请输入联系人姓名!'},],
									})(
						...
						<Form.Item label='最少投标人数量' extra="截标时投标人数量低于本设置将自动流标。">
							{getFieldDecorator('minBidderNum', {
								initialValue: detail.minBidderNum,
								rules: [
						...
						</Form.Item>
						{
							this.state.type ?
								<div className='prl8 text-center'>
									<Button type='primary' style={{width: '120px'}} size='large' htmlType='submit'
									        className='bg-primary-linear border-radius'>发布</Button>
									<Button type="primary"
									        size="large"
								...
								<div className='prl8 text-center'>
									<Button type='primary' style={{width: '120px'}} size='large' htmlType='submit'
									        className='bg-primary-linear border-radius'>重新发布</Button>
								</div>
						}
					</Form>
				</section>
...
const EditAddForm = Form.create()(EditInvite);
