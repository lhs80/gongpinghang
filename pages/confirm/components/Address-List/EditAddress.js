...
import {Row, Col, Form, Input, Select, Switch, Modal} from 'antd';
...
class EditAddressForm extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.addressInfo !== this.props.addressInfo) {
			let cities = this.state.data.filter(item => item.provinceName === nextProps.addressInfo.province);
			this.setState({
				...
				<Form onSubmit={this.handleSubmit}>
					<FormItem {...formItemLayout} label="收件人">
						{getFieldDecorator('userName', {
							initialValue: addressInfo.userName,
							rules: [
					...
					<FormItem {...formItemLayout} label="手机号码">
						{getFieldDecorator('userPhone', {
							initialValue: addressInfo.userPhone,
							rules: [
					...
					<FormItem {...formItemLayout} label="详细地址">
						{getFieldDecorator('address', {
							initialValue: addressInfo.address,
							rules: [
