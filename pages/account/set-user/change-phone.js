...
class ChangePhone extends React.Component {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				validateSmsCode(data).then(res => {
					if (res.result === 'success') {
						/*---提交表单---*/
						Router.push({pathname: '/account/set-user/new-phone', query: {mobile: phone}})
						// this.props.history.push(`/newPhone/${phone}`);
					} else {
						form.setFields({
							userCode: {
	...
	render() {
		...
		return (
			<Layout title="更换手机号" menuIndex={'3'} mainMenuIndex="setting">
...
const ChangePhoneForm = Form.create()(ChangePhone);
