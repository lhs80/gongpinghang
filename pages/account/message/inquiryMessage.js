...
export default class InquiryMessage extends React.Component {
	...
	delOneNews = () => {
		...
		if (this.state.type) {
			...
			newsDelAllFun(this.userCode, 1).then(res => {
				if (res.result === 'success') {
					this.queryMaterialMsg();
				}
			})
	...
	/*---所有消息设为已读---*/
	readNewsAll = () => {
		newsReadAllFun(this.userCode, 1).then(res => {
			if (res.result === 'success') {
				// this.queryMaterialMsg();
	...
	render() {
		return (
			<Layout mainMenuIndex="message" title="消息中心-询价消息" menuIndex={'1'}>
					...
					<Row style={{borderBottom: '1px solid #e6e6e6', margin: '0', paddingLeft: '34px', lineHeight: '60px'}} className="h4">
						...
						</Col>
						{
							this.state.list.length > 0 ?
								<Col span={12} className="text-right text-grey h5">
						...
						<List size="large" itemLayout="horizontal"
							...
							<List.Item
								...
								<List.Item.Meta
									...
									description={
										<a className={`mt1 h5 text-hover ${item.isRead === 1 ? 'text-muted' : 'text-grey'}`}
