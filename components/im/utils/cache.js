...
let Cache = (function() {
  ...
  Cache.prototype.getFriendAlias = function(account) {
    for (let i = this.friendslist.length - 1; i >= 0; i--) {
  ...
  Cache.prototype.addMsgsByReverse = function(msgs, reset) {
    ...
    for (let i = 0; i < msgs.length; i++) {
      if (msgs[i].scene === 'team') {
        user = msgs[i].to;
        let sessionId = msgs[i].scene + '-' + user
  ...
  Cache.prototype.dealTeamMsgReceipts = function(msgSymbol, callback) {
    ...
    if (typeof msgSymbol === 'string') {
      let msgs = this.msgs[msgSymbol] || [];
    } else {
      let sessionId = msgSymbol.sessionId;
      let msgs = this.msgs[sessionId] || [];
    }
    let teamMsgNeedReceipts = [];
    let teamMsgMap = {};
    let teamMsgReads = [];
    ...
    if (teamMsgNeedReceipts.length > 0) {
      nim.sendTeamMsgReceipt({
        teamMsgReceipts: teamMsgNeedReceipts,
          ...
          let failedCnt = content.teamMsgReceipts;
          if (failedCnt) {
            for (let i = 0; i < failedCnt.length; i++) {
      ...
      nim.getTeamMsgReads({
        teamMsgReceipts: teamMsgReads,
          ...
          if (content.teamMsgReceipts) {
            for (let i = 0; i < content.teamMsgReceipts.length; i++) {
              let cur = content.teamMsgReceipts[i];
  ...
  Cache.prototype.hasTeam = function(id) {
    let item;
    for (let i = this.teamlist.length - 1; i >= 0; i--) {
  ...
  Cache.prototype.removeTeamMembers = function(id, array) {
    let obj = this.teamMembers[id],
      account;
    if (obj) {
      for (let j = array.length - 1; j >= 0; j--) {
        account = array[j];
        for (let i = obj.members.length - 1; i >= 0; i--) {
  ...
  Cache.prototype.inMutelist = function(account) {
    for (let i = this.mutelist.length - 1; i >= 0; i--) {
  ...
  Cache.prototype.updatePersonSubscribe = function(data) {
    if (data.account) {
      ...
      let tempData = this.personSubscribes[account][data.type];
      ...
      if (+data.type === 1) {
        if (
          ...
          let serverConfig = JSON.parse(tempData.serverConfig);
          let customType = 0;
          ...
          if (tempData.custom && Object.keys(tempData.custom).length > 0) {
            let portStatus = getMultiPortStatus(customType, tempData.custom);
